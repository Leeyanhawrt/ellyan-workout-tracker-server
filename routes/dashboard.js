...
module.exports = (pool) => {
    router.get("/", authorization, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        try {
            const user = yield pool.query('SELECT first_name AS "firstName", last_name AS "lastName", email, id, workout_program_id AS "workoutProgramId" FROM users WHERE id = $1', [req.user]);
